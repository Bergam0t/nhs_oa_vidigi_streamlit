...
VENDORED COPY of streamlit-dnd 0.1.1 (PyPI), copied here verbatim.
Reason: the published wheel pins ``streamlit>=1.58``, which micropip refuses
to install under stlite (latest @stlite/browser 1.8.1 bundles Streamlit
1.57.0). The DOM contract this component relies on (``.st-key-*``,
``stElementContainer``, ``stLayoutWrapper``) has existed since Streamlit
1.50, so the pin is conservative rather than load-bearing. Vendoring skips
the pin entirely. ``pack.py`` links these files into the stlite build and
omits ``streamlit-dnd`` from its requirements list.
To update: re-copy from an installed ``streamlit_dnd/`` and re-check the
DOM selectors against the stlite-bundled Streamlit version.
